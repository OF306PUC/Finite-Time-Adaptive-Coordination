...
edgeProcess.on('message', (state) => {
    io.emit('state', state); 
    loggerLine(state); 
    console.log(`IO-Server-${params.node} Sent: x=${state.state}, z=${state.vstate}, ϑ=${state.vartheta}`);
})
...
async function shutdown(signal) {
    console.log(`[back] received ${signal}, cleaning up...`);
    try {
        if (params.trigger) await loggerEnd();
        edgeProcess.kill();
    } finally {
        process.exit(0);
    }
